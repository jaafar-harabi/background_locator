...
class BackgroundLocatorPlugin
  ...
  companion object {
    ...
    private fun registerLocator(
      ...
      if (IsolateHolderService.isServiceRunning) {
        Log.d(TAG, "Locator already running")
        result?.success(true)
        return
      }
      ...
      (args[Keys.ARG_INIT_CALLBACK] as? Long)?.let { handle ->
        InitPluggable().apply {
          ...
          (args[Keys.ARG_INIT_DATA_CALLBACK] as? Map<Any,Any>)?.let(this::setInitData)
        }
      }
    ...
    private fun startIsolateService(
      ...
      Intent(ctx, IsolateHolderService::class.java).apply {
        ...
        putExtra(Keys.SETTINGS_INTERVAL, settings[Keys.SETTINGS_INTERVAL] as? Int ?: 0)
        // …repeat for each setting with `as? T ?: default`
    ...
    private fun initializeService(ctx: Context, args: Map<Any,Any>) {
      (args[Keys.ARG_CALLBACK_DISPATCHER] as? Long)?.let {
        PreferencesManager.setCallbackHandle(ctx, Keys.CALLBACK_DISPATCHER_HANDLE_KEY, it)
      }
    }
    ...
    private fun updateNotificationText(ctx: Context, args: Map<Any,Any>) {
      Intent(ctx, IsolateHolderService::class.java).apply {
        action = IsolateHolderService.ACTION_UPDATE_NOTIFICATION
        (args[Keys.SETTINGS_ANDROID_NOTIFICATION_TITLE] as? String)?.let {
          putExtra(Keys.SETTINGS_ANDROID_NOTIFICATION_TITLE, it)
        }
      }.also { ContextCompat.startForegroundService(ctx, it) }
    }
  ...
  override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
    ...
    when (call.method) {
      ...
      Keys.METHOD_PLUGIN_UN_REGISTER_LOCATION_UPDATE -> unRegisterPlugin(context!!, result)
  ...
  override fun onNewIntent(intent: Intent): Boolean {
    ...
    val cb = PreferencesManager.getCallbackHandle(
